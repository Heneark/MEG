...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
% 2eme phase du prohramme
% est appel? par la fonction Adjust_Head_Pos_3DirbyCoil.m
% 
...
%
% - 19/01/17    RB
%      Correction du probl?me d'affichage si il n'y a pas d'event 
%
% - 15/02/17    RB
...
for xi_dsfile = 1 : length(Header.filename)
    ...
    if xi_dsfile == 1
        ...
        events = [events; extract_events(DS, CharLabel, Header.feat_events)+Header.blockoffset(xi_dsfile-1)];
    end
...
GUI.comput_distance = uicontrol('Parent', GUI.f, 'Style', 'pushbutton', 'String', 'Save Head Coils AND BadSegments as...',...
    'position', [500 10 200 20], 'Units', 'normalized',...
     'FontSize', 8, 'FontWeight','bold','BackgroundColor', [.7 0 .9],...
    'Callback', {@Extract_newHC_badsegments,false});
...
%si il n'y a pas d'event, on ne peut pas calculer le pourcentag d'event
%rejet?s
if ~isempty(ev)    

    % calcul du nombre d'evenements rejetes
    time_event = unique(ev.XData(~isnan(ev.XData)));
    
    [xxx, sample_event] = ismember(time_event, px.XData);
    clear xxx
...
%     time_event = ev.XData(seuil_event);
    ...
    % affichage des stats du nombre d'evenements rejetes
...
%     clear xi_cp copy_BS_file
    ...
    fid = fopen([Header.filename{xi_file}, separator, 'bad.segments']);
    old_bs = cell2mat(textscan(fid,'%f%f%f'));
    
    if ~isempty(old_bs)
        old_bs = Header.sample_rate*old_bs(:,2:3);else
    end
