function Adjust_Head_Pos_3DirbyCoil(DSpath)
...
% feat_events = Dysplay_menu_event();
Header.feat_events = Dysplay_menu_event();
...
for xi_dsfile = 1 : length(DSpath)
    ...
    if isempty(Header.blockoffset)
        events = extract_events(DS, CharLabel, Header.feat_events);
    else
        events = [events; extract_events(DS, CharLabel, Header.feat_events)+Header.blockoffset(end)];
    end
...
GUI.slide_delta_mov = uicontrol('Style', 'slider', 'Min', 0,'Max', 1, 'Value', 0.5,...      
...
% Selection du type d'events et le nom du fichier ou du channel CTF
function feat_events = Dysplay_menu_event()
...
f_event=figure('Position', [500 500 500 500], 'MenuBar', 'none', 'Tag', 'Menu_events');
...
uicontrol('Parent', f_event, 'Style', 'radiobutton', 'String', 'Files',...
    'Position', [300 420 100 20], 'FontSize', 12, 'FontWeight','bold',...
    'Tag', 'Select_file', 'Units', 'normalized', 'Callback', @Action_menu_event);

uicontrol('Parent',f_event, 'Style', 'frame',...
    'Position', [50 400 400 1])

uicontrol('Parent', f_event, 'Style', 'text', 'String', 'Channel Name:',...
    'Position', [50 375 100 20], 'Visible', 'off',...
    'FontSize', 12, 'FontWeight','bold',...
    'Tag', 'channel_txt', 'Units', 'normalized', 'Callback', '');
uicontrol('Parent', f_event, 'Style', 'edit', 'String', 'UPPT',...
    'Position', [50 345 100 20], 'Visible', 'off',...
    'FontSize', 12, 'FontWeight','bold',...
    'Tag', 'channel_inp', 'Units', 'normalized', 'Callback', '');

uicontrol('Parent',f_event, 'Style', 'frame',...
    'Position', [250 310 1 80])

uicontrol('Parent', f_event, 'Style', 'text', 'String', 'File Name:',...
    'Position', [300 375 100 20], 'Visible', 'off',...
    'FontSize', 12, 'FontWeight','bold',...
    'Tag', 'file_txt','Units', 'normalized', 'Callback', '');
uicontrol('Parent', f_event, 'Style', 'edit', 'String', 'MarkerFile.mrk',...
    'Position', [300 345 100 20], 'Visible', 'off',...
    'FontSize', 12, 'FontWeight','bold',...
    'Tag', 'file_inp', 'Units', 'normalized', 'Callback', '');

uicontrol('Parent', f_event, 'Style', 'pushbutton', 'String', 'Select',...
    'Position', [230 280 40 20], 'FontSize', 12, 'FontWeight','bold',...
    'Tag', 'Select', 'Units', 'normalized', 'Callback', @Action_menu_event);


fprintf('Select events input\n')
waitfor(f_event)


% Selection du type d'events et le nom du fichier ou du channel CTF
function feat_events = Action_menu_event(gcbo, eventdata, handles);
...
switch gcbo.Tag
    ...
    case 'Select_CTF_chan'
        if gcbo.Value == 1
            set(findobj('Tag', 'Select_file'), 'Value', 0);
            set(findobj('Tag', 'file_txt'), 'Visible', 'off');
            set(findobj('Tag', 'file_inp'), 'Visible', 'off');
            set(findobj('Tag', 'channel_txt'), 'Visible', 'on');
            set(findobj('Tag', 'channel_inp'), 'Visible', 'on');
        elseif gcbo.Value == 0
            set(findobj('Tag', 'channel_txt'), 'Visible', 'off');
            set(findobj('Tag', 'channel_inp'), 'Visible', 'off');
        end

    case 'Select_file'
        if gcbo.Value == 1
            set(findobj('Tag', 'Select_CTF_chan'), 'Value', 0);
            set(findobj('Tag', 'channel_txt'), 'Visible', 'off');
            set(findobj('Tag', 'channel_inp'), 'Visible', 'off');
            set(findobj('Tag', 'file_txt'), 'Visible', 'on');
            set(findobj('Tag', 'file_inp'), 'Visible', 'on');
        elseif gcbo.Value == 0
            set(findobj('Tag', 'file_txt'), 'Visible', 'off');
            set(findobj('Tag', 'file_inp'), 'Visible', 'off');
        end
    ...
    case 'Select'
        ...
        if get(findobj('Tag', 'Select_CTF_chan'), 'Value')
            feat_events.name = get(findobj('Tag', 'channel_inp'), 'String');
            feat_events.type = 'CTF_channel';            
        elseif get(findobj('Tag', 'Select_file'), 'Value')            
            feat_events.name = strmatch(get(findobj('Tag', 'file_inp'), 'String'), CharLabel);
            feat_events.type = 'File';
        end
        
    delete(findobj('Tag', 'Menu_events'))
...
function trigg_events = extract_events(DS, CharLabel, feat_events)
    
switch feat_events.type
    
    case 'CTF_channel'
        Index_event = strmatch(feat_events.name, CharLabel);
