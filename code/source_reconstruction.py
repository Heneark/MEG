...
def src_rec15(task, subject, state, block_group, evoked=dict(), noise_cov=None, keys=['R','T'], name='ECG_included', window={'R':(.35,.5), 'T':(.1,.25)}, surface='ico4', volume=6.2, bem_spacing='ico4', baseline_cov=True, mindist=5, method:"'beamformer', 'MNE', 'dSPM', 'sLORETA', 'eLORETA'"="dSPM"):
    """
    If compute_fwd=True, compute and save forward solution (overwriting previously existing one).
    If compute_inv=True, compute and save inverse solution.
    If compute_stc=True, compute and save SourceEstimate.
    ...
    if surface:
        ...
        if method is 'beamformer':
            for k in keys:
                data_cov = mne.read_cov(op.join(cov_path, '{}_{}_{}-{}_{}-data-cov.fif'.format(subject, state, '_'.join(block_group), k, name)))
                filters = mne.beamformer.make_lcmv(mne.pick_channels_evoked(evoked[k], exclude=bad_chan).info, mne.pick_channels_forward(fwd_surf, exclude=bad_chan), mne.pick_channels_cov(data_cov, exclude=bad_chan), noise_cov=mne.pick_channels_cov(noise_cov, exclude=bad_chan), pick_ori=None)
                
                # Compute SourceEstimate
                stc_surf_file = op.join(stc_path, '{}_{}_{}-{}_{}-{}-surface_{}-{}_{}-{}'.format(subject, state, '_'.join(block_group), k, name, ('baseline_cov' if baseline_cov else 'empty_room_cov'), surface, *window[k], method))
                stc_surf = mne.beamformer.apply_lcmv(mne.pick_channels_evoked(evoked[k], exclude=bad_chan), filters)
                stc_surf.save(stc_surf_file)
